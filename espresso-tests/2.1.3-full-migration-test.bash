...
emph ./test-node.bash --simple --init-force --detach --no-build-utils
if [ "$DEBUG" = "true" ]; then
  ...
  echo
  ./test-node.bash --simple --init-force --detach --no-build-utils > "$TESTNODE_LOG_FILE" 2>&1
fi
...
# Disabling references to the tokenbridge for this test as it currently isn't needed
...
forge script --chain $PARENT_CHAIN_CHAIN_ID scripts/foundry/contract-upgrades/espresso-2.1.3/DeployEspressoNitroContracts2Point1Point3UpgradeAction.s.sol:DeployEspressoNitroContracts2Point1Point3UpgradeActionScript --rpc-url $PARENT_CHAIN_RPC_URL --broadcast -vvvv --sender $OWNER_ADDRESS --private-key $PRIVATE_KEY

export UPGRADE_ACTION_ADDRESS=$(cat broadcast/DeployEspressoNitroContracts2Point1Point3UpgradeAction.s.sol/1337/run-latest.json | jq -r '.transactions[-1].contractAddress'| cast to-checksum)
# Echo for debugging.
echo "Deployed Celestia migration action at $UPGRADE_ACTION_ADDRESS"

cd $ORBIT_ACTIONS_DIR

forge script --chain $PARENT_CHAIN_CHAIN_ID scripts/foundry/contract-upgrades/espresso-2.1.3/ExecuteEspressoNitroContracts2Point1Point3Upgrade.s.sol:ExecuteEspressoNitroContracts2Point1Point3UpgradeScript --rpc-url $PARENT_CHAIN_RPC_URL --broadcast -vvvv --sender $OWNER_ADDRESS --private-key $PRIVATE_KEY
