...
# if [ "$DEBUG" = "true" ]; then
#   "$TESTNODE_DIR/test-node.bash" --espresso $(get_espresso_image_flag) --caff-node --tokenbridge --init-force --detach --no-simple
# else
...
#   echo
#   "$TESTNODE_DIR/test-node.bash" --espresso $(get_espresso_image_flag) --validate --tokenbridge --caff-node --init-force --detach --no-simple > "$TESTNODE_LOG_FILE" 2>&1
# fi
