#!/usr/bin/env bash
# Usage
#   ./deploy-to-sequencer.sh env-file [other arguments]
# Example
#   ./deploy-to-sequencer.sh nitro-1.env --init --latest-espresso-image --no-tokenbridge --validate
#
set -exo pipefail
...
num_volumes=$(docker volume ls --filter label=com.docker.compose.project=$COMPOSE_PROJECT_NAME -q | wc -l)
...
if $force_init; then
    ...
    docker-compose down
    leftoverContainers=$(docker container ls -a --filter label=com.docker.compose.project=$COMPOSE_PROJECT_NAME -q | xargs echo)
    ...
    leftoverVolumes=$(docker volume ls --filter label=com.docker.compose.project=$COMPOSE_PROJECT_NAME -q | xargs echo)
    ...
    # If multiple compose projects are running on the same host, the default L2
    # URL ws://sequencer:8548 is ambiguous. Therefore we use an L2 URL with the
    # full name of the docker container.
    docker-compose run scripts bridge-funds --l1url $L1_URL_WS --l2url ws://${COMPOSE_PROJECT_NAME}-sequencer-1:8548 --ethamount 100000 --wait
    docker-compose run scripts bridge-funds --l1url $L1_URL_WS --l2url ws://${COMPOSE_PROJECT_NAME}-sequencer-1:8548 --ethamount 1000 --wait --from "key_0x$devprivkey"
