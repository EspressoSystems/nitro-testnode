...
async function bridgeNativeToken(
  ...
  const depositAmount = BigNumber.from(argv.amount).mul(
  ...
  if (argv.wait) {
    ...
    if (decimals < 18) {
      ...
      expectedMintedAmount = depositAmount.mul(
    ...
    while (true) {
      ...
      if (
        bridgerBalanceAfter.sub(bridgerBalanceBefore).eq(expectedMintedAmount)
      ) {
        return;
      }
      await sleep(100);
...
export const transferL3ChainOwnershipCommand = {
  ...
  handler: async (argv: any) => {
    ...
    const [, , , , , , , l3UpgradeExecutorAddress] =
      await tokenBridgeCreator.inboxToL2Deployment(inboxAddr);
    ...
    argv.data = arbOwnerIface.encodeFunctionData("addChainOwner", [
      l3UpgradeExecutorAddress,
    ]);
    await runStress(argv, sendTransaction);

    // remove L3 owner from chain owners
    argv.data = arbOwnerIface.encodeFunctionData("removeChainOwner", [
      namedAccount("l3owner").address,
    ]);
    await runStress(argv, sendTransaction);
...
export const transferERC20Command = {
  ...
  builder: {
    ...
    dryrun: {
      boolean: true,
      describe: "true if only simulate (default: false)",
      default: false,
    },
  },
  handler: async (argv: any) => {
    ...
    const amountToTransfer = BigNumber.from(argv.amount).mul(
      BigNumber.from("10").pow(tokenDecimals),
    );
    const to = namedAccount(argv.to).address;

    if (argv.dryrun) {
      console.log("Simulating transfer-erc20");
      const result = await tokenContract.callStatic.transfer(
        to,
        amountToTransfer,
      );
      console.log(result);
      ...
      const txResponse = await tokenContract.transfer(to, amountToTransfer);
...
export const getERC20BalanceCommand = {
  ...
  handler: async (argv: any) => {
    ...
    const tokenContract = new ethers.Contract(argv.token, ERC20.abi, provider);
    const accountAddr = namedAccount(argv.account).address;
    const balance = await tokenContract.balanceOf(accountAddr);
    console.log("balance of " + accountAddr + " is: " + balance);
...
export const sendRPCCommand = {
  ...
  builder: {
    ...
    params: { object: true, describe: "array of parameter name/values" },
  },
  handler: async (argv: any) => {
    const rpcProvider = new ethers.providers.JsonRpcProvider(argv.url);
    const params =
      typeof argv.params === "string" ? JSON.parse(argv.params) : argv.params;

    const result = await rpcProvider.send(argv.method, params);
